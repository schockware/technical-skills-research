...
**6. The imposter-syndrome figure is corroborating, not load-bearing — and it is not "accurate self-assessment."** The 52.7% figure (Clance scale, 2024) is flagged ⬜ unverified; even verified, elevated imposter rates appear across all high-achievement/high-evaluation populations (healthcare runs *higher* — ~62%) regardless of rotation structure, so "imposter syndrome is the engineer accurately perceiving structural mismatch" is correlation read as causation (`R1` Claim H). It is *consistent with* the thesis; it does not prove it. The ~50% reversion rate carries the behavioral weight. *(Propagation status, 2026-06-27: `01_` line 94 and `06_` [heading + body] now both carry the "consistent with, not accurate self-assessment" framing — this reconciliation is complete. The earlier note that "01_/06_ still state the strong causal version" is no longer accurate.)*
