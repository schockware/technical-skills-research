...
The founding CTO is asked to span three **distinct, clearly-defined** skill sets — **Builder** (product/craft) → **Multiplier** (people/team) → **Strategist** (market/board) — with **no on-the-job runway to learn the next before being judged against it.** Each transition is a **transformation** (offload the old role + acquire a new one), not a promotion. Failure clusters *at the boundaries between modes*, not within them. The dev-ladder research matters because **this is not unique to the CTO** — it's the terminal, concentrated expression of a mismatch that begins at the *first* promotion (Junior→Mid) and runs the whole ladder. The CTO just gets it worst: all three modes, pre-scale, alone, in 18–24 months instead of 15+ years. *(Note: earlier phrasings said "irrational composite / sequentially-incompatible / kind mismatch" — all **retired**; the skills are not inherently incompatible [see `18_RESEARCH`], the role is **under-resourced**. See the README retirement callout + `R3` must-not-claim table.)*
...
| `16_RESEARCH_success_cases.md` | **★ The success search collapsed the category.** Looked for a founding CTO who navigated all 3 modes as a continuous tenure → **found none** (in a thin, n≈5 anecdotal sample). Every "success" = 1–2 modes + graceful exit. *Consistent with rarity, not "survivor-proof"* — the term was retracted (R1 Claim 3, R2 F1; lead with `AX-REVERSION`, n≈30,000, not this). Three patterns: external naming was always required; success = selective scope-narrowing not full replacement; terminal outcome is usually departure. |
