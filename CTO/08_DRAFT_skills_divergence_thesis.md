...
A scope mismatch is survivable: it asks for the same skill, sooner and with less support. A kind mismatch asks one person, pre-scale, to span two skill tracks the industry itself split into separate ladders — sequentially, three times, with no runway to learn each before being judged on it. ⚠️ ⟦**RETIRED claim** — the original sentence continued "…the industry declared *incompatible*… a *dominated demand, irrational* in the precise sense that no version of meeting it is rational." All of that is retired: the skills are *distinct, not incompatible* (`18_RESEARCH`); the role is *under-resourced, not irrational/dominated* (`R3` must-not-claim, README callout). This whole file is superseded as a build target — see top banner.⟧
...
> A Builder doing an individual-contributor job was given a Strategist title to satisfy an investor pattern-recognition heuristic — then measured against that title's implied expectations at the next round, with no support structure to get there. **The pattern is consistent with investors who required the signal helping to create the structural problem they were trying to avoid.** *(Retired un-hedged form, cut per `RISK-INVESTOR-CAUSAL`/R1/R2: "the irrational composite was institutionalized by the very party whose job is to evaluate execution risk." It overclaimed "irrational" and asserted causation the evidence does not measure — see the hedge in the paragraph below.)* <!-- APPENDIX-REF: AX-INVESTOR — investor-signaling causal chain; dot-com "management by press release." Causal-narrative, not measured study — see framing caution in 07_APPENDIX. -->
