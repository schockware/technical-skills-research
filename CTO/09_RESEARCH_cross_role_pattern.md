...
> For every other C-suite role, the core skill *does not rotate.* The CFO at seed does what the CFO at Series B does: manage money — scope expands, focus holds. The CMO owns the market narrative at every stage — channels scale, direction holds. **Only the CTO's focus rotates** — product → people → market — because the IC/management bifurcation exists in no other discipline (established in `08_DRAFT` §2a, the M3=❌ mitigation gap).
...
3. **It sharpens "irrational" (open question #3) to a precise formal sense.** Not "irrational = hard." Irrational = *the industry imported a job description across a context boundary it doesn't survive, and for the CTO specifically across a skill-kind boundary the industry itself had already declared uncrossable (the two tracks).* That is a dominated move: there is no version of "one person, pre-scale, spanning two formally-incompatible skill tracks sequentially" that is rational.
...
1. **The IC/management bifurcation is unique to engineering.** No other function formally declared its two skill tracks incompatible enough to build separate career ladders. Sales has no IC track. Marketing doesn't. PM is ambiguous. Only engineering said, structurally, "these skills diverge — choose." *Then we ask the CTO to hold both.* (This is the M3=❌ in its strongest form.)
...
- The study is of **founders/entrepreneurs generally (the CEO-analog)** — the buffered case (M1). It does not study the CTO, where the IC/management bifurcation (M3=❌) makes the "adopt a new role identity" step a jump across a track the industry itself declared incompatible.
