...
>
> **The pattern is consistent with investors who required the signal helping to create the structural problem they were trying to avoid.** By demanding a "CTO" rather than a "technical co-founder," they imported a title carrying expectations the role was not yet resourced to meet — and the person was then evaluated against those expectations. *(Hedged per `RISK-INVESTOR-CAUSAL`, R1, and R2: this is a causal **narrative assembled from signaling evidence, not a measured causal study.** The retired un-hedged form — "the irrational composite was institutionalized by the very party whose job is to evaluate execution risk" — overclaimed both "irrational" and the causal direction; R1/R2 said cut it. The defensible claim is that the evidence is **consistent with** the title functioning as an investor signal, not that investors **caused** the failure.)*
...
2. **It connects to the scope-vs-kind distinction cleanly.** Every C-suite title is a scale-mismatch when migrated pre-scale; the CTO's version is the most *acute* because the IC→management rotation it spans is the sharpest skill discontinuity. *(Retired: the stronger "only the CTO is a **kind**-mismatch" framing — `R3` must-not-claim table demoted this from a uniqueness claim to a severity claim; the same under-resourced-rotation pattern runs the whole ladder, see `18_RESEARCH` and `../Software Developer/`.)* This finding answers *why the migration happened at all* — investor signaling — and is consistent with why it was never corrected: correcting it would mean removing a credibility signal investors rely on. The incentive to keep the misnomer is structural.
