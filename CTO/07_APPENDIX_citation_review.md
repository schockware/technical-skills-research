...
- **Second verification (2026-06-25) — the CRO genuinely ROTATES, not just scales.** Industry sources describe VP-Sales→CRO as "fundamentally different from a typical promotion" (execution → revenue-system: cross-functional strategy, marketing alignment, retention), and document sales leaders *failing* the transition for lack of the new skills — the same failure pattern as the CTO. **This retired the corpus's stale "only the CTO's focus rotates" overclaim** (`09_RESEARCH` line 43, now flagged superseded per `R3` must-not-claim). The CRO is the CTO's **closest C-suite equivalent** (same craft→strategy rotation, same founding/board context) — which is what makes "even the nearest peer got the split, eng didn't" load-bearing rather than incidental.
